...
namespace vv {
	...
	void VvRenderer::beginOffscreenRenderPass(VkCommandBuffer commandBuffer) {
		VkRenderPassBeginInfo renderPassInfo{};
		renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
		renderPassInfo.renderPass = vvSwapChain->getOffscreenPass().renderPass;
		renderPassInfo.framebuffer = vvSwapChain->getOffscreenPass().framebuffer;
		renderPassInfo.renderArea.offset = { 0, 0 };
		renderPassInfo.renderArea.extent = vvSwapChain->getOffscreenPass().extent;
		...
		viewport.x = 0.0f;
		viewport.y = 0.0f;
		viewport.width = static_cast<float>(vvSwapChain->getOffscreenPass().extent.width);
		viewport.height = static_cast<float>(vvSwapChain->getOffscreenPass().extent.height);
		viewport.minDepth = 0.0f;
		viewport.maxDepth = 1.0f;
		VkRect2D scissor{ {0, 0}, vvSwapChain->getOffscreenPass().extent };
		vkCmdSetViewport(commandBuffer, 0, 1, &viewport);
		vkCmdSetScissor(commandBuffer, 0, 1, &scissor);
	}
	...
	VkDescriptorImageInfo VvRenderer::getOffscreenDescriptor() const {
		return vvSwapChain->getOffscreenPass().descriptor;
	}
}